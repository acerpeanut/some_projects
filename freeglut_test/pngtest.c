...
png_const_bytep
create_buff(png_structp png_ptr, png_infop info_ptr, int width, int height)
{
   //PNG_CONST unsigned int bit_depth = png_get_bit_depth(png_ptr, info_ptr);
   png_const_bytep buff=NULL;
   unsigned int size_raw = width*height;

   switch (png_get_color_type(png_ptr, info_ptr))
   {
      case PNG_COLOR_TYPE_GRAY:
         //printf("GRAY %u\n", component(row, x, 0, bit_depth, 1));
         buff = (png_const_bytep)malloc(size_raw*1);
         return buff;
      ...
      case PNG_COLOR_TYPE_PALETTE:
         {
               ...
               PNG_INFO_PLTE) && num_palette > 0 && palette != NULL)
            {
               ...
               if ((png_get_tRNS(png_ptr, info_ptr, &trans_alpha, &num_trans,
                  NULL) & PNG_INFO_tRNS) && num_trans > 0 &&
                  trans_alpha != NULL)
               	  buff = (png_const_bytep)malloc(size_raw*4);
                  //printf("INDEXED %u = %d %d %d %d\n", index,
                  //   palette[index].red, palette[index].green,
                  //   palette[index].blue,
                  //   index < num_trans ? trans_alpha[index] : 255);
                  ...
                  //   palette[index].red, palette[index].green,
                  //   palette[index].blue);
                  buff = (png_const_bytep)malloc(size_raw*3);
            }
      ...
      case PNG_COLOR_TYPE_GRAY_ALPHA:
         //printf("GRAY+ALPHA %u %u\n", component(row, x, 0, bit_depth, 2),
         //   component(row, x, 1, bit_depth, 2));
         buff = (png_const_bytep)malloc(size_raw*2);
         return buff;
